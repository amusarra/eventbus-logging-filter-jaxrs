 ...
 *
 * <p>
 * L'indicazione dell'indirizzo virtuale dell'evento per la richiesta HTTP è definita nel
...
public class HttpResponseConsumer {
    ...
    void onStart(@Observes StartupEvent ev) {
        ...
        consumer = eventBus.consumer(httpResponseVirtualAddress);
    ...
    void onStop(@Observes ShutdownEvent ev) {
        if (consumer != null) {
            ...
            log.debugf(
                    "Unregistering the consumers to the event bus for HTTP response at addresses: {%s}",
                    httpResponseVirtualAddress);
        }
    }
     ...
     *
     * <p>
     * Il metodo riceve il messaggio dell'evento e invia il messaggio al Dispatcher per la
